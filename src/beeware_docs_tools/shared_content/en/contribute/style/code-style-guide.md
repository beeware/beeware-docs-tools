...
Declaring an abstract base class as formally inheriting from [`ABC`](https://docs.python.org/3/library/abc.html) (and decorating abstract methods with `@abstractmethod`) introduces a small but nonzero runtime overhead from enforcing that subclasses implement the necessary methods. Therefore, we only do so for base classes intended to be subclassed by users; for internal use, there's no need to inherit from `ABC`, and abstract methods should raise `NotImplementedError` instead of being decorated.
